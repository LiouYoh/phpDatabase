...
if (isset($Confirm)) {   // 確認按鈕
    ...
    // Demo for XSS
       $Name = xsspurify($Name);
       $Phone = xsspurify($Phone);
	   $Address = xsspurify($Address);
	   $comment = xsspurify($comment);
	   $gender = xsspurify($gender);
	   $birthday = xsspurify($birthday);
    // Demo for the reason to use addslashes
        ...
        $sqlcmd="UPDATE namelist SET name='$Name',phone='$Phone',groupid='$GroupID', "
            . "address='$Address',comment='$comment',gender='$gender',birthday='$birthday' WHERE cid='$cid'";
...
<tr height="30">
  <th>生日</th>
  <td><input type="text" name="birthday" value="<?php echo $birthday ?>" size="20"></td>
</tr>
