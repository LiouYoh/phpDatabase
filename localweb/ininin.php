...
function GetLYear($CurDate) { 
	...
	$today = strtotime($CurDate); 
    for($i=0;$i<10;$i++){
		if($today >= strtotime($LunerYearMap[$i][0])){
			if($today <= strtotime($LunerYearMap[$i][1])){
				return $LunerYearMap[$i][2];
			}
...
for($r=0;$r<6;$r++){
	...
	for($c=0;$c<7;$c++){
		...
		$d = str_pad($arrMonth[$r][$c],2,'0',STR_PAD_LEFT);
		$really = "$year-$m-$d";	
		if($arrMonth[$r][$c] == date("d") && $month == date("m")){
			if($a>=1 && $a<=$LastDate)
				echo '<td bgcolor="#0000ff"> ' . $arrMonth[$r][$c] .GetLYear($really)."<br>".GetLDay($really).'</td>';
			else
				echo '<td bgcolor="#0000ff"> ' . $arrMonth[$r][$c] ."<br>".'</td>';
		}
		else if($c ==6 || $c == 0){
			if($a>=1 && $a<=$LastDate)
				echo '<td bgcolor="#00FF00"> ' . $arrMonth[$r][$c] ."<br>".GetLDay($really).'</td>';
			else
				echo '<td bgcolor="#00ff00"> ' . $arrMonth[$r][$c] ."<br>".'</td>';
		}
		else{
			if($a>=1 && $a<=$LastDate)
				echo '<td>' . $arrMonth[$r][$c]."<br>" .GetLDay($really). '</td>';
			else
				echo '<td > ' . $arrMonth[$r][$c] ."<br>".'</td>';
		}
	}
	echo '</tr>';
}
